...
export BACKUP_ROOTDIR="/opt/backup"

# MySQL user and password.
export MYSQL_USER="db_user"
export MYSQL_PASSWD="db_pass"
...
${CMD_MYSQL} -u"${MYSQL_USER}" -h"${MYSQL_HOST}" -p"${MYSQL_PASSWD}" -e "show databases" &>/dev/null
if [ X"$?" != X"0" ]; then
...
for db in ${DATABASES}; do
    ...
    ${CMD_MYSQL} -u"${MYSQL_USER}" -h"${MYSQL_HOST}" -p"${MYSQL_PASSWD}" -e "use ${db}" &>/dev/null
    ...
    if [ X"$?" == X'0' ]; then
        ...
        if [ X"$?" == X'0' ]; then
            ...
            sql_log_msg="INSERT INTO sys_backup (event, level, msg, admin, ip, date_created) VALUES ('backup', 'info', 'Database backup: ${db}. Original file size: ${original_size}, compressed: ${compressed_size}, backup file: ${compressed_file_name}', 'cron_backup_sql', '127.0.0.1', NOW());"
        else
            # backup failed
            sql_log_msg="INSERT INTO sys_backup (event, level, msg, admin, ip, date_created) VALUES ('backup', 'info', 'Database backup failed: ${db}, check log file ${LOGFILE} for more details.', 'cron_backup_sql', '127.0.0.1', NOW());"
        fi

        # Log to SQL table `iredadmin.log`, so that global domain admins can
        # check backup status (System -> Admin Log)
        ${CMD_MYSQL} -u"${MYSQL_USER}" -h"${MYSQL_HOST}" -p"${MYSQL_PASSWD}" ${DATABASES} -e "${sql_log_msg}"
    fi
done
